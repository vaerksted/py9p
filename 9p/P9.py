...
DIR = 020000000000
QDIR = 0x80
OREAD, OWRITE, ORDWR, OEXEC = list(range(4))
...
class Marshal(object):
    ...
    def _enc8(self, x):
        "Encode a 4-byte integer"
        self._checkSize(x, 0xffffffffffffffff)
        self._enc4(x & 0xffffffff)
        self._enc4(x >> 32)
...
class Marshal9P(Marshal):
    ...
    def _decTwalk(self):
        ...
        newfid = self._dec4()
        l = self._dec2()
        names = [self._decS() for n in range(l)]
    ...
    def _decRwalk(self):
        l = self._dec2()
        return [self._decQ() for n in range(l)]
