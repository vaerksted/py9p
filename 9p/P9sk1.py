...
AuthTreq, AuthChal, AuthPass, AuthOK, AuthErr, AuthMod = list(range(1, 7))
AuthTs, AuthTc, AuthAs, AuthAc, AuthTp, AuthHr = list(range(64, 70))
...
def makeKey(password):
    ...
    buf = list(password)
    while 1:
        t = list(map(ord, buf[:8]))

        k = [(((t[i]) >> i) + (t[i+1] << (8-(i+1))) & 0xff) for i in range(7)]
